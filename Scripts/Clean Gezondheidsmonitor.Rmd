...
The purpose of this document is to clean and explore the Gezondheidsmonitor dataset. This is a Dutch cross-sectional dataset based on a large, health-related questionnaire administered by the Community Health Services, Statistics Netherlands and the National Institute for Public Health and the Environment [@GGD2012]. The dataset contains self-reported weight and height measurements, rounded to nearest kilogram and centimetre. The included survey weights allow for representative analyses of the Netherlands. The survey was conducted in September, October and November of 2012, and was repeated in 2016 and in 2020.
...
Let's load the Gezondheidsmonitor dataset. Throughout the document, we will limit the output in each step. This is because the Gezondheidsmonitor data are not open source. To request access to the data, please visit [www.monitorgezondheid.nl](https://www.monitorgezondheid.nl).
...
```{r}
gezondheidsmonitor.data <- gezondheidsmonitor.data %>%
  mutate(bmi = ifelse(bmi > 70, NA, bmi)) # Filter in main
```
