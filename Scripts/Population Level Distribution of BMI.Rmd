...
In the chunk below, we have limited the output. This is because the Public Health Monitor dataset is not open source. To request access to the data, please visit [www.monitorgezondheid.nl](https://www.monitorgezondheid.nl).
...
The Public Health Monitor questionnaire was repeated in 2016 and we can use these data for external validation. Given that our model is stratified by sex, level of education and age, many demographic variations will already be accounted for. But the external validation will check

```{r}
model.bmi.by.sex.education.and.age.2016 <- bmi.data %>%
  filter(
    year == 2016,
    age >= 20,
    age < 95
...
model.bmi.by.sex.education.and.age.2016
```

After the modelled prevalences, the observed prevalences can be determined.

```{r}
observed.bmi.by.sex.education.and.age.2016 <- bmi.data %>%
  filter(
    year == 2016,
    age >= 20,
    age < 95
  ...
  ungroup()
observed.bmi.by.sex.education.and.age.2016
```
...
ggplot() +
  geom_col(
    ...
    data = observed.bmi.by.sex.education.and.age.2016
  ) +
  geom_point(
    mapping = aes(
      x = ifelse(sex == 0, -100 * prevalence, 100 * prevalence),
    ...
    data = model.bmi.by.sex.education.and.age.2016,
    size = 0.8
  ) +
  scale_x_continuous(
    labels = abs,
