...
Let's check if the background data covers everyone who filled in the health questionnaire.
...
```{r}
liss.data %>%
  count(year(date), participant.id) %>%
  count(year(date), n, name = "participants")
```
...
```{r}
liss.data %>%
  mutate(wave.year = if_else(wave == 200802, 2007, year(date))) %>%
...
Now that the background data has been combined with the health data, we can filter out specific rows from the data applying to minors.
