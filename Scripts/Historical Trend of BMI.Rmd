...
Historical data related to BMI were assessed by Statistics Netherlands and the National Institute for Public Health and the Environment [@CBS2021]. We use the prevalences of overweight and obesity for adults between 1990 and 2021 which were standardised to the demography of the Netherlands in 2021 [@VZinfo2021]. Due to this standardisation step, any trend we extract from these data will mostly be indicative of changes in lifestyle and its influence on BMI rather than also including trends in demography. The dataset loaded here is a cleaned version, which was processed in the document [_Clean-VZinfo.html_](/R:/Projecten/V02001214 KV2.1.B CZmodellering/3. Uitvoering/3. Data Voorbewerkt/VZinfo/Clean-VZinfo.html), where the data are also explored in more detail.

```{r}
bmi.by.year.and.sex <- read_csv(
  file = paste0(input.data.path, "/Risk Factors/BMI by Year and Sex.csv"),
  col_types = cols(
    year = col_integer(),
    sex = col_factor(),
    bmi = col_factor(),
    standardised = col_factor(),
...
We can now find the values for $\mu$ and $\sigma$ which correspond to the observed prevalences, by year and sex, keeping the $\nu$ and $\tau$ parameters fixed. Given that there are two unknowns and two data points, this calculation should give an exact result. An easy method to obtain this result is to minimise the squared difference between the observed prevalences and those implied by certain values for $\mu$ and $\sigma$.
...
```{r}
ggsave(
  file = "R:\\Projecten\\V02001214 KV2.1.B CZmodellering\\6. Producten\\2022-08-15 - Mathematical Medicine and Biology - A Model of Individual BMI Trajectories\\Figures\\Historical Trend.pdf",
  width = 260,
  height = 130,
  units = "mm"
)
...
Let's also create a plot of the resulting BMI distribution by year and sex. As previously mention, the trend in overweight and obesity is, in part, due to an increase in the spread of the BMI distribution, which can clearly be seen in [figure (3)](#fig:BMI-Distribution-by-Year).
