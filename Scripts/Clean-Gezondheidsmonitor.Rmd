...
Let's load the Gezondheidsmonitor dataset. The survey was first conducted in September, October and November of 2012, and was repeated in 2016 and in 2020. However, the data for 2020 are likely to be affected by the COVID pandemic, which is not an effect we would like to include. So we will combine the first two datasets here. Throughout the document, we will limit the output in each step. This is because the Gezondheidsmonitor data are not open source. To request access to the data, please visit [www.monitorgezondheid.nl](https://www.monitorgezondheid.nl).
...
gezondheidsmonitor.data <- gezondheidsmonitor.data %>%
  mutate(
    education = ifelse(education == 1, education, education - 1),
