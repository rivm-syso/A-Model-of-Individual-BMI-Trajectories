...
Historical data about BMI were assessed by Statistics Netherlands and the National Institute for Public Health and the Environment~\citep{CBS2021}. We use the prevalences of overweight and obesity for adults between 1990 and 2021 which were standardised to the demography of the Netherlands in 2021~\citep{VZinfo2021}. Table~\ref{tab:VZinfo} shows some of the figures from this dataset.
...
Male & 18.05 & 17.75 & 17.20 & 0.2573 & -0.002137\\
Female & 19.00 & 18.29 & 17.61 & 0.1560 & -0.001174\\
\tblhead{& $\sigma_{education}^{low}$ & $\sigma_{education}^{med}$ & $\sigma_{education}^{high}$ & $\sigma_{age}$ & $\sigma_{age^2}$ \\[-9.5pt]}\\[-9.5pt]
Male & 2.210 & 1.924 & 1.677 & 0.03491 & -0.0003327\\
Female & 2.728 & 2.398 & 1.960 & 0.03460 & -0.0002926\\
...
Our longitudinal data could, in principle, be used to estimate both a population level distribution and a historical trend of BMI. This would only work if these data are representative of the Dutch population. We do not want to make this assumption and prefer to rely on the Public Health Monitor and the VZinfo datasets to model the population level distribution and a historical trend of BMI. The benefit is that this separates our model into independent parts. Whatever trend is contained in the longitudinal data, this should largely be removed when transforming the BMI values to z-scores. Nonetheless, if these data are not representative, this procedure need not result in values with zero mean and unit standard deviation; some bias may remain. Ultimately, we want a model which suits the entire population, so which produces true z-scores. A simple solution is to model the bias by including a fixed intercept and fixed slope in our statistical analysis, as shown in equation~\ref{eq:Mixed-Effects Model}. We are not interested in the values of this intercept and slope, but by including the terms, the other parameters are not compromised.

\subsection{Results}
\label{sec:Individual Trajectories/Results}

The estimated values, excluding the fixed intercept and slope, are listed in table~\ref{tab:Mixed-Effects Model Parameters}.
...
Male & 0.4744 & 34.53 & 0.4761 & 0.04951 \\
Female & 0.6287 & 19.21 & 0.3323 & 0.03905 \lastline
\end{tabular}
\label{tab:Mixed-Effects Model Parameters}
}}
\end{table}
...
% Complexities will arise when this model is connected to the risk of disease, given that in older age, weight loss can be an indication of an underlying condition, but that is for a subsequent paper.
% het effect van BMI op ziekten niet lineair is. Daardoor hoort bij een periode laag-gemiddeld-hoog toch een net iets ander risico als alleen constant gemiddeld (TODO: ook toevoegen aan artikel)
...
% The main rationale for developing the model is to simulate the effects of prevention policies.  Given this motivation, I would have expected to see more emphasis on how the model would behave when (hypothetical) policies are simulated.
% From my understanding the forecasts at the population level are not in any way influenced by the underlying individual dynamics.
% I would also be interested in how the authors would use the model to evaluate polices.
% How can we translate effects of interventions (for instance measured in trails) into changes in model parameters?
% Ons BMI model wordt niet alleen gebruikt om het referentiescenario door te rekening, maar ook om de effecten van interventies te schatten. Het feit dat het BMI model uit twee componenten bestaat, biedt de mogelijkheid om twee verschillende type interventies te modelleren. Zo is de eerste optie om een interventie te modelleren welke een invloed heeft op het bevolkingsniveau. Dit kan gerealiseerd worden door de coëfficiënten van de BMI verdeling aan te passen. Maar ons model biedt ook de mogelijkheid om fijnmazigere interventies door te rekenen. Zo is de tweede optie om een interventie toe te passen welke de levensloop van een individu verandert.
% het effect van BMI op ziekten niet lineair is. Daardoor hoort bij een periode laag-gemiddeld-hoog toch een net iets ander risico als alleen constant gemiddeld (TODO: ook toevoegen aan artikel)

\section{Conclusion}
\label{sec:Conclusion}

% Conclusion: in the absence of better information, using the longitudinal parameters from this study elsewhere can be justified, but it does come with the assumption that the same trend applies over time.
